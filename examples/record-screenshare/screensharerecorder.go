...
func main() {
	...
	panic(session.MakeWebsocketConnection(func(session *zoom.ZoomSession, message zoom.Message) error {
		...
		case *zoom.JoinConferenceResponse:
			// TODO(hackish): move this elsewher
			streams, err = zoom.CreateZoomScreenShareStreams(session)
			if err != nil {
				return err
			}
		...
		case *zoom.SharingStatusIndication:
			...
			resolution := 4 // 1 works
			session.SharingSubscribeRequest(m.ActiveNodeID, resolution)
			return nil
